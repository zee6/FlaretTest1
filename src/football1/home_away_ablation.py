...
from football1.home_away import HomeAwayRow, build_home_away_rows
...
PLUS_VENUE = "frozen_residual_plus_home_away_v1"
VARIANTS = (BASELINE, PLUS_VENUE)
...
def home_away_map(db_path: Path) -> dict[str, HomeAwayRow]:
    return {row.match_id: row for row in build_home_away_rows(db_path)}


def variant_feature_names(variant: str) -> tuple[str, ...]:
    if variant == BASELINE:
        return FEATURE_NAMES
    if variant == PLUS_VENUE:
        return FEATURE_NAMES + tuple(f"home_away__{name}" for name in VENUE_FEATURE_NAMES)
...
def variant_vector(
    row: FeatureRow,
    variant: str,
    venue_by_match: dict[str, HomeAwayRow],
) -> list[float]:
    ...
    if variant == PLUS_VENUE:
        try:
            venue = venue_by_match[row.match_id]
        except KeyError as exc:
            raise ValueError(f"Missing home/away row for {row.match_id}") from exc
        return base + [float(getattr(venue, name)) for name in VENUE_FEATURE_NAMES]
    raise ValueError(f"Unknown variant: {variant!r}")
...
def fit_variant(
    ...
    *,
    ...
    x_raw = np.asarray(
        [variant_vector(row, variant, venue_by_match) for row in rows],
        dtype=float,
    )
    ...
    if not result.success:
        raise RuntimeError(f"Home/away ablation optimization failed: {result.message}")
...
def predict_variant(
    ...
    x = model.scaler.transform(
        [variant_vector(row, model.variant, venue_by_match)]
    )[0]
    logits = np.log(base)
...
def walk_forward_home_away_ablation(
    db_path: Path,
    *,
    ...
    venue_by_match = home_away_map(db_path)
    ...
    for test_index in range(min_train_seasons, len(seasons)):
        ...
        for variant in VARIANTS:
            model = fit_variant(
                train,
                variant=variant,
                venue_by_match=venue_by_match,
                alpha=alpha,
            )
            items = [
                (predict_variant(model, row, venue_by_match), row.result)
                for row in test
            ]
    ...
    for variant in VARIANTS:
        ...
        overall[variant] = {
            **metrics,
            ...
            "log_loss_delta_vs_raw": _delta(
                metrics["log_loss"], raw["log_loss"]
            ),
            "brier_delta_vs_raw": _delta(metrics["brier"], raw["brier"]),
        }

    baseline = overall[BASELINE]
    plus_venue = overall[PLUS_VENUE]
    return {
        "experiment": "market_anchored_home_away_ablation_v1",
        "market_source": "B365 pre-closing, de-vigged immutable offset",
        "alpha": alpha,
        "baseline_policy": "the original inspected fixed-market residual v1 is preserved exactly; no switch to the post-hoc no-Elo variant",
        "hyperparameter_policy": "alpha=0.10 and home/away v1 feature definitions fixed before this OOS ablation; no OOS tuning",
        "comparison": "same original residual architecture and features; plus-home-away adds exactly four venue-role features",
        ...
        "incremental": {
            "plus_home_away_minus_baseline_log_loss": _delta(
                plus_venue["log_loss"], baseline["log_loss"]
            ),
            "plus_home_away_minus_baseline_brier": _delta(
                plus_venue["brier"], baseline["brier"]
            ),
            "plus_home_away_minus_baseline_ece": _delta(
                plus_venue["top_label_ece"], baseline["top_label_ece"]
            ),
        },
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Test venue-role features inside the frozen market residual."
    )
    parser.add_argument(
        "--database",
        type=Path,
...
def main() -> None:
    ...
    report = walk_forward_home_away_ablation(
        args.database,
    ...
    if args.output:
        ...
        print(f"wrote home/away ablation report to {args.output}")
    else:
        print(text)
