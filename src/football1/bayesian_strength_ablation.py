...
from football1.bayesian_strength import BayesianStrengthRow, build_bayesian_strength_rows
...
def bayesian_map(db_path: Path) -> dict[str, BayesianStrengthRow]:
    return {row.match_id: row for row in build_bayesian_strength_rows(db_path)}


def variant_feature_names(variant: str) -> tuple[str, ...]:
    if variant == "legacy_baseline":
        return FEATURE_NAMES
    if variant == "plus_bayesian_strength":
        return FEATURE_NAMES + (
            "bayes_expected_goal_diff",
            "bayes_latent_diff_sd",
        )
    if variant == "plus_bayesian_regime":
        return FEATURE_NAMES + (
            "bayes_expected_goal_diff",
            "bayes_latent_diff_sd",
            "bayes_regime_diff",
...
def variant_vector(
    row: FeatureRow,
    variant: str,
    bayes_by_match: dict[str, BayesianStrengthRow],
) -> list[float]:
    values = [float(getattr(row, name)) for name in FEATURE_NAMES]
    if variant == "legacy_baseline":
        return values
    try:
        bayes = bayes_by_match[row.match_id]
    except KeyError as exc:
        raise ValueError(f"Missing Bayesian state for {row.match_id}") from exc
    values.extend([bayes.expected_goal_diff, bayes.latent_diff_sd])
    if variant == "plus_bayesian_strength":
        return values
    if variant == "plus_bayesian_regime":
        return values + [
            bayes.regime_diff,
            abs(bayes.home_regime) + abs(bayes.away_regime),
        ]
    raise ValueError(f"Unknown variant: {variant!r}")
...
def fit_variant(
    ...
    *,
    ...
    x_raw = np.asarray(
        [variant_vector(row, variant, bayes_by_match) for row in rows], dtype=float
    )
    ...
    if not result.success:
        raise RuntimeError(f"Bayesian ablation optimization failed: {result.message}")
...
def predict_variant(
    ...
    x = model.scaler.transform(
        [variant_vector(row, model.variant, bayes_by_match)]
    )[0]
    logits = np.log(base)
...
def walk_forward_bayesian_ablation(
    db_path: Path,
    *,
    ...
    bayes_by_match = bayesian_map(db_path)
    ...
    for test_index in range(min_train_seasons, len(seasons)):
        ...
        for variant in VARIANTS:
            model = fit_variant(
                train,
                variant=variant,
                bayes_by_match=bayes_by_match,
                alpha=alpha,
            )
            items = [
                (predict_variant(model, row, bayes_by_match), row.result)
                for row in test
            ]
    ...
    legacy = overall["legacy_baseline"]
    strength = overall["plus_bayesian_strength"]
    regime = overall["plus_bayesian_regime"]

    return {
        "experiment": "market_anchored_dynamic_bayesian_ablation_v1",
        "market_source": "B365 pre-closing, de-vigged immutable offset",
        "alpha": alpha,
        "hyperparameter_policy": (
            "residual alpha and Bayesian v1 state parameters frozen before this "
            "ablation; no OOS tuning"
        ),
        "comparison": (
            "same frozen market-anchored residual architecture and legacy football "
            "features; Bayesian pre-match state is appended only in candidate variants"
        ),
        "bayesian_strength_features": [
            "pre-match expected goal difference",
            "pre-match posterior uncertainty of the team-strength difference",
        ],
        "regime_features": [
            "pre-match home-minus-away exponentially weighted standardized surprise",
            "pre-match combined absolute surprise activity",
        ],
        "same_day_policy": (
            "Bayesian and legacy features snapshot all fixtures on a date before "
            "same-date result updates"
        ),
        "overall_raw_market": raw,
        "overall_variants": overall,
        "incremental_comparisons": {
            "bayesian_strength_vs_legacy_log_loss": _delta(
                strength["log_loss"], legacy["log_loss"]
            ),
            "bayesian_regime_vs_legacy_log_loss": _delta(
                regime["log_loss"], legacy["log_loss"]
            ),
            "bayesian_regime_vs_strength_log_loss": _delta(
                regime["log_loss"], strength["log_loss"]
            ),
            "bayesian_strength_vs_legacy_brier": _delta(
                strength["brier"], legacy["brier"]
            ),
            "bayesian_regime_vs_legacy_brier": _delta(
                regime["brier"], legacy["brier"]
            ),
            "bayesian_regime_vs_strength_brier": _delta(
                regime["brier"], strength["brier"]
            ),
        },
        "seasons": reports,
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Ablate dynamic Bayesian strength inside the fixed-market residual model."
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.output:
        ...
        print(f"wrote Bayesian ablation report to {args.output}")
    else:
        print(text)
